...
def test_select_pos_neg_methods():
    for method in ["GMM", "kmeans", "ward"]:
        row = np.array([1, 1, 1, 10, 10, 10])
        mask_pos, mask_neg, snr, size, is_converged = binarization.select_pos_neg(row, min_n_samples=2, method=method)
        assert mask_pos.sum() == 3
        assert mask_neg.sum() == 3
        assert snr > 0
        assert size == 3
...
def test_binarize_minimal(monkeypatch, tmp_path):
    ...
    binarized, stats, null_dist = binarization.binarize(
        str(tmp_path / 'testbin'), exprs=data, save=False, load=False, 
        plot_all=False, verbose=False, min_n_samples=3, n_permutations=100
    )
...
def test_binarize_save_load(tmp_path):
    ...
    assert len(created_files) >= 2, f"Expected at least 2 files, got {len(created_files)}: {[f.name for f in created_files]}"
    ...
    assert len(background_files) == 1, f"Expected 1 background file, got {len(background_files)}"
...
def test_binarize_plot(monkeypatch, tmp_path):
    ...
    binarized, stats, null_dist = binarization.binarize(
        str(tmp_path / 'testbin'), exprs=data, save=False, load=False, 
        plot_all=True, verbose=False, min_n_samples=3, n_permutations=100, 
        show_fits=['geneA', 'geneB'], method="GMM", seed=42
    )
    assert isinstance(binarized, pd.DataFrame)
    assert isinstance(stats, pd.DataFrame)
    assert null_dist is not None
