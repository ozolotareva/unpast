...
def _validate_cols(bics: pd.DataFrame, score_col: str | None = None) -> pd.DataFrame:
    ...
    if score_col is not None:
        ...
        df["score"] = pd.to_numeric(df[score_col]).astype('float64')  # validate SNR is numeric
...
def _calc_average_precision_by_matrix(mat_iou_pred_to_gt: np.ndarray, thresh: float) -> float:
    """Calculate Average Precision (AP) for predicted biclusters vs. ground truth
    ...
    assert 0.0 <= thresh <= 1.0, f"IoU / Jaccard thresholds must be in [0.0, 1.0], found {thresh}"
...
def calc_average_precision_at_thresh(
    ...
    """
    if len(bics_true) == 0:
        print("No ground-truth biclusters provided, returning AP=0.0 or 1.0 based on predictions.")
