...
def _validate_cols(bics: pd.DataFrame, use_SNR: bool) -> pd.DataFrame:
    ...
    if use_SNR:
        ...
        df["SNR"] = pd.to_numeric(df["SNR"]).astype(
            "float64"
...
def calculate_bicluster_similarity(
    bic1: pd.Series, bic2: pd.Series, dimension: str = "both"
) -> Dict[str, float]:
...
def calculate_map_at_iou_range(
    ...
    for iou_threshold in iou_thresholds:
        ap = calculate_average_precision(bics_true, bics_pred, iou_threshold, dimension)
        ap_scores.append(ap)
    ...
    result = {f"mAP@{start_iou:.1f}-{end_iou:.2f}": map_score, **ap_by_threshold}
...
def calculate_size_metrics(bics_df: pd.DataFrame) -> Dict[str, float]:
    ...
    n_genes = (
        bics_df["n_genes"]
        if "n_genes" in bics_df.columns
        else bics_df["genes"].apply(len)
    )
    n_samples = (
        bics_df["n_samples"]
        if "n_samples" in bics_df.columns
        else bics_df["samples"].apply(len)
    )
...
def calculate_quality_metrics(
    bics_df: pd.DataFrame, data: Optional[pd.DataFrame] = None
) -> Dict[str, float]:
    ...
    if bics_df.empty:
        return {"avg_snr": 0.0, "median_snr": 0.0, "max_snr": 0.0, "min_snr": 0.0}
    ...
    if data is not None:
        logger.info(
            "Expression data provided but MSR/variance calculations not implemented yet"
        )
...
def calculate_metrics(
    ...
    metrics.update({"pred_" + k: v for k, v in quality_pred.items()})
    ...
    if not bics_true.empty and not bics_pred.empty:
        ...
        if true_to_pred:
            avg_jaccard = np.mean(
                [match["similarity"] for match in true_to_pred.values()]
            )
...
def calculate_metrics_on_df(
    ...
    for idx, row in df.iterrows():
        bics_true = (
            row[true_col]
            if true_col in row and not pd.isna(row[true_col])
            else pd.DataFrame()
        )
        bics_pred = (
            row[pred_col]
